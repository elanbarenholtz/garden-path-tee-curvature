...
The present study assesses whether human reading times reflect such directional sensitivity beyond what surprisal captures. We compute trajectory extrapolation error from the hidden states of transformer language models and ask whether it contributes to reading-time prediction \new{beyond surprisal. An affirmative result would indicate that directional dynamics constitutes a dissociable dimension of processing cost rather than a redundant summary of word-level predictability.}
...
\new{Both of those corpora use self-paced reading, which delivers words serially, one at a time and only on request. Reading with free eye movements does not: parafoveal information about a word is available before it is fixated. Speech, the modality for which comprehension presumably evolved, affords no such preview either. A measure defined over the step-by-step evolution of a representation might therefore apply to serial presentation specifically rather than to reading in general. We test this by extending the analysis to two eye-tracking corpora, ZuCo \citep{hollenstein2018} and OneStop \citep{berzak2025}, the latter larger than either self-paced corpus and well powered to detect processing-cost effects.}
...
\new{\paragraph{Reading-time prediction.} Trajectory extrapolation error predicts self-paced reading time beyond surprisal and lexical controls ($\Delta\text{AIC} = 112$, $\beta = +0.0035$, $p = 4.0 \times 10^{-26}$, $N = 813{,}621$, 178 participants). For scale, in the same model $\beta(\text{surprisal}) = +0.0112$ and $\beta(\text{log frequency}) = +0.0072$: the trajectory effect is roughly a third the magnitude of surprisal, and is a second-order effect on reading time rather than a competitor to it.}

\new{\paragraph{Subject-level inference.} Because a pooled model over 813,621 observations can register an effect that no individual reader exhibits, we estimated the model separately within each participant. Of 171 participants with sufficient data, 125 (73.1\%) showed a positive coefficient (sign test $p = 1.2 \times 10^{-9}$; Wilcoxon $p = 5.1 \times 10^{-12}$; $t(170) = 7.46$), and the mean per-participant coefficient ($+0.0039$) closely matches the pooled estimate. Thirty-nine participants reached individual significance. We adopt this form of inference throughout, and take 65\% sign agreement together with a group-level $p < .01$ as the criterion for a reliable effect.}

\new{\paragraph{Robustness.} The effect survives controls for word identity, centring the outcome and all predictors within word type (2,919 types occurring five or more times): it attenuates by roughly 40\% but remains ($\Delta\text{AIC} = 23.1$, $\beta = +0.0022$, $p = 5.3 \times 10^{-7}$). A substantial share of the raw effect is therefore lexical, and a trajectory-specific component survives. Adding a punctuation covariate slightly strengthens it ($\Delta\text{AIC} = 115.4$), and restricting to punctuation-free words strengthens it further ($\Delta\text{AIC} = 138.3$, $\beta = +0.0041$).}
...
\paragraph{Regression.} A mixed-effects model including both $z$-scored surprisal and $z$-scored extrapolation error confirmed independent \new{contributions from each ($\beta_{\text{surprisal}} = +0.0113$, $\beta_{\text{extrapolation}} = +0.0035$). We are not able to say whether those contributions are additive. A model including their interaction fits marginally better than the additive model ($\Delta\text{AIC} = 1.8$ in favour of the interaction; interaction $\beta = -0.00056$, $p = .051$), a difference too small to support a claim in either direction on 812,730 observations. The negative sign indicates that if anything the trajectory effect is slightly attenuated at high surprisal, but we treat the two models as indistinguishable here and draw no conclusion about how the measures combine.}
...
In ZuCo \citep{hollenstein2018}, 12 participants reading isolated sentences, extrapolation error predicted total reading time in 11 of 12 participants ($\beta = +0.0079$, sign test $p = .006$), consistent in direction with the self-paced results; first fixation duration was null. \new{Entering surprisal and lexical controls alongside it gives a similar result (10 of 12 positive, $\beta = +0.0124$, $p = .009$).} With only 12 participants this is weak evidence, and the result is sensitive to specification: an earlier analysis of the same data with untransformed durations and no surprisal control returned a null. \new{Surprisal itself is not reliable in this corpus (8 of 12 participants for total reading time, sign test $p = .39$), so ZuCo is underpowered for the comparison that matters and we do not rest anything on it.}

In OneStop \citep{berzak2025}, a substantially larger and better-powered corpus, the effect did not replicate. In the ordinary-reading subcorpus (180 participants, 1,104,883 observations), first fixation and gaze duration were null. \new{Total reading time was reliably negative, and this is not a null but a reversal: the coefficient is negative under every specification we ran, ranging from $\beta = -0.0023$ ($p = .029$) using the corpus's own surprisal annotations and a context-based trajectory measure, to $\beta = -0.0060$ ($p = 1.5 \times 10^{-8}$, 54 of 180 participants positive) under the full control set. Adding previous-word length, frequency and surprisal, previous dwell time, or the previous word's extrapolation error does not change this. We report the most conservative of these estimates in what follows, but the direction is consistent and the stronger specifications are the better-controlled ones.} The corpus is demonstrably sensitive to processing-cost effects: in the same data and specification, surprisal predicted total reading time in 178 of 180 participants ($\beta = +0.031$, $p = 2.9 \times 10^{-31}$).
