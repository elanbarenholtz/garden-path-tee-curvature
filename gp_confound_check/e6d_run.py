"""
E6d -- Switchboard confirmation tier (PREREG_E6_production_latency
Amendment 5, committed before the corpus was downloaded).

Transitions: consecutive words, same speaker, same utterance, only
[silence] between; excluded if any other-channel word onset falls inside
the silence. DV = log(1 + silence ms), clip [0, 5000]. Context: both
channels onset-ordered per conversation. Cluster = conversation side
(>= 200 transitions); criterion Wilcoxon p < .01 AND >= 65% positive.
Surface normalization: [laughter-X] -> X; alternation _N stripped;
partial-word brackets deleted ("reci[pe]-" -> "reci"); bare
[silence]=SIL; [noise]/[vocalized-noise]/[laughter]/<markers> break
floor-holding and are non-words.

Usage: e6d_run.py prepare | measures | analyze
"""
import os, re, sys, glob
import numpy as np
import pandas as pd
from scipy import stats
from wordfreq import zipf_frequency

sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from e5_rank_test import GP, GPC, first_write_ranges, load_model

SWB = f"{GP}/switchboard/swb_ms98_transcriptions"
TRANS_CSV = f"{GPC}/e6d_transitions.csv"
TEXTS_CSV = f"{GPC}/e6d_texts.csv"
MEAS_CSV = f"{GPC}/e6d_word_measures.csv"


def norm_token(tok):
    """Returns (kind, surface): kind in {'word','sil','noise'}."""
    if tok == "[silence]":
        return "sil", ""
    if tok.startswith("<") or tok in ("[noise]", "[vocalized-noise]",
                                      "[laughter]"):
        return "noise", ""
    m = re.fullmatch(r"\[laughter-(.+)\]", tok)
    if m:
        tok = m.group(1)
    tok = re.sub(r"_\d+$", "", tok)
    tok = re.sub(r"\[[^\]]*\]", "", tok).strip("-")
    if not tok or tok.startswith("["):
        return "noise", ""
    return "word", tok


def parse_side(path):
    out = []
    for ln in open(path, encoding="latin-1"):
        p = ln.split()
        if len(p) < 4:
            continue
        utt, a, b = p[0], float(p[1]), float(p[2])
        kind, surf = norm_token(" ".join(p[3:]))
        out.append({"utt": utt, "on": a, "off": b, "kind": kind,
                    "surf": surf})
    return out


def prepare():
    convs = {}
    for f in glob.glob(f"{SWB}/*/*/sw*-ms98-a-word.text"):
        m = re.search(r"sw(\d{4})([AB])-", os.path.basename(f))
        if m:
            convs.setdefault(m.group(1), {})[m.group(2)] = f
    print(f"{len(convs)} conversations")
    trows, texts = [], []
    for cid in sorted(convs):
        sides = {ch: parse_side(f) for ch, f in convs[cid].items()}
        pool = []
        for ch, ents in sides.items():
            for e in ents:
                if e["kind"] == "word":
                    e["ch"] = ch
                    pool.append(e)   # shared dict: word_pos visible in ents
        if len(pool) < 200:
            continue
        pool.sort(key=lambda r: (r["on"], r["ch"]))
        for i, r in enumerate(pool):
            r["word_pos"] = i
        texts.append({"conv": cid, "n_words": len(pool),
                      "text": " ".join(r["surf"] for r in pool)})
        starts = np.array([r["on"] for r in pool])
        chs = [r["ch"] for r in pool]
        for ch, ents in sides.items():
            run = 0
            for k in range(1, len(ents)):
                b = ents[k]
                if b["kind"] != "word":
                    continue
                # find previous word in this side; check what intervenes
                j = k - 1
                ok_sil = True
                while j >= 0 and ents[j]["kind"] != "word":
                    if ents[j]["kind"] != "sil":
                        ok_sil = False
                    j -= 1
                if j < 0:
                    continue
                a = ents[j]
                if not ok_sil or a["utt"] != b["utt"]:
                    run = 0
                    continue
                run += 1
                gap = (b["on"] - a["off"]) * 1000.0
                if gap < 0 or gap > 5000:
                    continue
                lo = np.searchsorted(starts, a["off"], "right")
                hi = np.searchsorted(starts, b["on"], "left")
                if any(chs[q] != ch for q in range(lo, hi)):
                    continue
                trows.append({"conv": cid, "speaker": f"{cid}{ch}",
                              "word_pos": b["word_pos"], "gap_ms": gap,
                              "run_pos": run})
    pd.DataFrame(texts).to_csv(TEXTS_CSV, index=False)
    D = pd.DataFrame(trows)
    D.to_csv(TRANS_CSV, index=False)
    print(f"{len(texts)} conversations kept, "
          f"{sum(t['n_words'] for t in texts):,} words, "
          f"{len(D):,} transitions")
    print(f"gap: =0 {(D.gap_ms==0).mean():.1%}  >200 ms "
          f"{(D.gap_ms>200).mean():.1%}  median-of-positive "
          f"{D[D.gap_ms>0].gap_ms.median():.0f} ms")
    spk = D.groupby("speaker").size()
    print(f"speakers (conv-sides): {len(spk)}; >= 200: {(spk>=200).sum()}")
    print("DV SANITY GATE:",
          "PASS" if (D.gap_ms > 200).mean() > 0.02 else "FAIL")


def measures():
    import torch
    tok, model = load_model("gpt2", "cpu")
    T = pd.read_csv(TEXTS_CSV, dtype={"conv": str})
    first = not os.path.exists(MEAS_CSV)
    done = set()
    if not first:
        done = set(pd.read_csv(MEAS_CSV, usecols=["conv"],
                               dtype={"conv": str}).conv.unique())
    for r in T.itertuples():
        if r.conv in done:
            continue
        text = r.text
        words = text.split()
        spans, c = [], 0
        for w in words:
            spans.append((c, c + len(w))); c += len(w) + 1
        enc = tok(text, return_offsets_mapping=True)
        ids, offs = enc["input_ids"], enc["offset_mapping"]
        n = len(ids)
        bpe_word = np.full(n, -1); wi = 0
        for bi, (cs, ce) in enumerate(offs):
            while cs < ce and text[cs].isspace():
                cs += 1
            if ce <= cs:
                continue
            while wi < len(spans) and cs >= spans[wi][1]:
                wi += 1
            if wi < len(spans) and cs >= spans[wi][0] and ce <= spans[wi][1]:
                bpe_word[bi] = wi
        first_sub = np.full(len(words), -1); last_sub = np.full(len(words), -1)
        for bi, w in enumerate(bpe_word):
            if w >= 0:
                if first_sub[w] < 0:
                    first_sub[w] = bi
                last_sub[w] = bi
        if not (first_sub >= 0).all():
            print(f"{r.conv}: unmapped words -- skipped")
            continue
        H = np.full((n, 768), np.nan, dtype=np.float32)
        surp = np.full(n, np.nan)
        F = {k: np.full(n, np.nan) for k in ("ent", "ren", "t1", "t10")}
        written = np.zeros(n, bool)
        tt = torch.tensor(ids)
        for p, end, lo, hi in first_write_ranges(n):
            with torch.no_grad():
                o = model(tt[p:end].unsqueeze(0), output_hidden_states=True)
            hs = o.hidden_states[6][0].float().numpy()
            lp = torch.log_softmax(o.logits[0].double(), dim=-1).numpy()
            for g in range(p, end):
                if not written[g]:
                    H[g] = hs[g - p]; written[g] = True
            for t in range(max(lo, 1), hi):
                if np.isnan(surp[t]):
                    row = lp[t - 1 - p]
                    surp[t] = -row[ids[t]]
                    pr = np.exp(row)
                    F["ent"][t] = float(-(pr * row).sum())
                    F["ren"][t] = float(-np.log((pr ** 2).sum()))
                    F["t1"][t] = float(row.max())
                    F["t10"][t] = float(np.log(np.sort(pr)[-10:].sum()))
            del o
        out = []
        for w in range(len(words)):
            t, fs = last_sub[w], first_sub[w]
            tee = np.nan
            curv3 = np.nan
            if t - 3 >= 1:
                W = H[t - 3:t].astype(np.float64)
                A = np.column_stack([np.ones(3), np.arange(3.0)])
                cf, *_ = np.linalg.lstsq(A, W, rcond=None)
                tee = float(np.linalg.norm(H[t] - (cf[0] + cf[1] * 3)))
            if t - 4 >= 1:
                # curvature_3 (PREREG_E9): mean angle between successive
                # token steps over the last 3 steps ending at t
                seg = np.diff(H[t - 4:t + 1].astype(np.float64), axis=0)
                nn = np.linalg.norm(seg, axis=1)
                if (nn > 1e-9).all():
                    cosv = (seg[1:] * seg[:-1]).sum(1) / (nn[1:] * nn[:-1])
                    curv3 = float(np.arccos(np.clip(cosv, -1, 1)).mean())
            out.append({"conv": r.conv, "word_pos": w, "tee": tee, "curv3": curv3,
                        "surprisal": (np.nansum(surp[fs:t + 1])
                                      if fs >= 1 else np.nan),
                        "f_entropy": F["ent"][fs], "f_renyi2": F["ren"][fs],
                        "f_top1": F["t1"][fs], "f_top10": F["t10"][fs],
                        "word_length": len(words[w]),
                        "zipf": zipf_frequency(words[w].lower(), "en")})
        pd.DataFrame(out).to_csv(MEAS_CSV, mode="a", header=first,
                                 index=False)
        first = False
        print(f"{r.conv} done", flush=True)


def analyze():
    D = pd.read_csv(TRANS_CSV, dtype={"conv": str})
    M = pd.read_csv(MEAS_CSV, dtype={"conv": str})
    D = D.merge(M, on=["conv", "word_pos"], how="left")
    D["dv"] = np.log1p(D.gap_ms)
    D["l_run"] = np.log1p(D.run_pos)
    D["cum_words"] = np.log1p(D.word_pos)
    PRED = os.environ.get("E6_MEASURE", "tee")
    CTRL = ["surprisal", "f_entropy", "f_renyi2", "f_top1", "f_top10",
            "zipf", "word_length", "l_run", "cum_words"]
    use = ["dv", PRED] + CTRL
    D = D[np.isfinite(D[use]).all(axis=1)].copy()
    for c in use:
        D[c] = D[c] - D.groupby("speaker")[c].transform("mean")
    print(f"usable transitions: {len(D):,}   conv-sides "
          f"{D.speaker.nunique()}")
    betas, ns = [], []
    for sp, s in D.groupby("speaker"):
        if len(s) < 200:
            continue
        X = np.column_stack([np.ones(len(s))] +
                            [(s[c] - s[c].mean()).values /
                             (s[c].std() if s[c].std() > 0 else 1)
                             for c in [PRED] + CTRL])
        try:
            b, *_ = np.linalg.lstsq(X, s.dv.values, rcond=None)
        except np.linalg.LinAlgError:
            continue
        betas.append(b[1]); ns.append(len(s))
    betas = np.array(betas)
    pos = (betas > 0).mean()
    w = stats.wilcoxon(betas)
    print(f"[predictor: {PRED}]")
    print(f"\nconv-sides >= 200: {len(betas)} "
          f"(median n {int(np.median(ns))})")
    print(f"TEE coefficient: mean {betas.mean():+.5f}  %pos {pos:.1%}  "
          f"Wilcoxon p {w.pvalue:.2e}")
    ok = (w.pvalue < .01) and (pos >= .65)
    print(f"\nPREREGISTERED CRITERION (Amendment 5): "
          f"{'PASS' if ok else 'FAIL'}")
    print(f"descriptive: raw r(tee, dv) "
          f"{np.corrcoef(D[PRED], D.dv)[0, 1]:+.4f}")


if __name__ == "__main__":
    {"prepare": prepare, "measures": measures,
     "analyze": analyze}[sys.argv[1]]()
