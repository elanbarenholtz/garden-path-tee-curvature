...
OUT_DIR = f"{GP}/gp_confound_check"
...
def story_pass(text):
    ...
    while pos < n:
        ...
        if end >= n:
            break
        pos += STRIDE
    return hidden, hidden12, offsets, n
...
def dir_pres_rows(H, last_sub, n_words, ws, sid, layer_name):
    W = np.array([H[last_sub[i]] for i in range(n_words)])
    out = []
    for i in range(ws + 1, W.shape[0]):
        pre = W[i - ws:i]
        m = pre.shape[0]
        A = np.column_stack([np.ones(m), np.arange(m)])
        c, *_ = np.linalg.lstsq(A, pre, rcond=None)
        d = c[1]; dn = np.linalg.norm(d)
        if dn < 1e-10:
            continue
        du = d / dn
        row = {"story_id": sid, "layer": layer_name, "window": ws}
        for ahead in [0, 1, 2, 3]:
            j = i + ahead
            if j < W.shape[0]:
                delta = W[j] - W[j - 1]
                nn = np.linalg.norm(delta)
                if nn > 1e-10:
                    row[f"ahead{ahead}"] = abs(float(np.dot(delta, du) / nn))
        out.append(row)
    return out


def displacement(hidden, ls, prev_ls):
    """Raw magnitude of representational change, no direction:
       step  = ||h[ls] - h[ls-1]||              (last BPE step)
       wdisp = ||h[ls] - h[prev word's ls]||    (word-to-word)
       hnorm = ||h[ls]||                        (state magnitude)"""
    step = np.nan
    if ls in hidden and (ls - 1) in hidden:
        step = float(np.linalg.norm(hidden[ls] - hidden[ls - 1]))
    wdisp = np.nan
    if prev_ls is not None and ls in hidden and prev_ls in hidden:
        wdisp = float(np.linalg.norm(hidden[ls] - hidden[prev_ls]))
    hnorm = float(np.linalg.norm(hidden[ls])) if ls in hidden else np.nan
    return step, wdisp, hnorm
...
for sid in story_ids:
    ...
    hidden, hidden12, offsets, n_bpe = story_pass(text)
    ...
    for w in range(len(spans)):
        ...
        dstep, dword, hnorm = displacement(hidden, ls,
                                           last_sub[w - 1] if w > 0 else None)
        ...
        rows.append({"story_id": sid, "word_idx": w, "final_bpe_re": ls,
                     ...
                     "disp_step": dstep, "disp_word": dword,
                     "state_norm": hnorm})
    for Hd, lname in [(hidden, "L6"), (hidden12, "L12")]:
        for ws in (3, 5):
            dp_rows.extend(dir_pres_rows(Hd, last_sub, len(spans), ws, sid, lname))
...
print(f"  disp_word mean/sd:        {M.disp_word.mean():.3f} / {M.disp_word.std():.3f}")
print(f"  r(disp_word, tee_k3):     {M.disp_word.corr(M.tee_k3):.4f}")
print(f"  r(disp_word, tee3_perp):  {M.disp_word.corr(M.tee3_perp):.4f}")

DP = pd.DataFrame(dp_rows)
DP.to_csv(f"{OUT_DIR}/dirpres_8a6087341e.csv", index=False)
print("\n" + "="*72)
print("TABLE 4 (recomputed on verified states): direction preservation")
print("="*72)
print(f"{'layer':>6}{'window':>8}{'current':>10}{'+1':>9}{'+2':>9}{'+3':>9}{'n':>10}")
for (L, ws), g in DP.groupby(["layer", "window"]):
    print(f"{L:>6}{ws:>8}{g.ahead0.mean():>10.3f}{g.ahead1.mean():>9.3f}"
          f"{g.ahead2.mean():>9.3f}{g.ahead3.mean():>9.3f}{len(g):>10,}")
